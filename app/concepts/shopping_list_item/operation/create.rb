...
class ShoppingListItem::Operation::Create < Base::Operation::Base
  def perform!(params:, current_user:)
    shopping_list = ShoppingList.find(params[:shopping_list_id])
    item = shopping_list.shopping_list_items.build(item_params(params))
    ...
    if item.save
      ...
      notice I18n.t("shopping_list_items.create.success")
    else
      self.redirect_path = shopping_list_path(shopping_list)
  ...
  def item_params(params)
    if params.is_a?(ActionController::Parameters)
      ...
      params.fetch(:shopping_list_item, {}).slice(:name).with_indifferent_access
    end
  end
